...
export function read(life: Life, path: string, on = life.root): { path: string; content: string } {
 ...
 * answered. */
export function write(life: Life, path: string, content: string, on = life.root): { path: string; content: string } {
...
export function bash(life: Life, command: string, kwargs: Record<string, unknown> = {}, on = life.root): string {
