...
async def test_what_the_act_it_is_at_came_to_as_the_record_stands_where_the_call_is_made() -> None:
  ...
  await settle()
  running = engine.peek(act, on=root)
  assert isinstance(running, Exit) and running.code is None and running.stdout.content == "half\n"
  engine.send("exited", act, 0, by=WORLD)
  assert await act == engine.peek(act, on=root)
...
async def test_the_done_of_that_act_filled_its_outcome_and_the_life_holds_every_outcome_by_name() -> None:
  ...
  engine.send("out", act, "half\n", "stdout", by=WORLD)
  await settle()
  told = engine.peek(act, on=root)
  assert isinstance(told, Exit) and (told.code, told.stdout.content) == (None, "half\n")
  step = engine.rung("close(21)", on=root)
  await step
  assert engine.outcomes[step] == 21 == engine.peek(step, on=root)
  engine.send("exited", act, 0, by=WORLD)
  await settle()
  assert engine.peek(act, on=root) == engine.outcomes[act]
...
async def test_a_peek_at_a_name_of_no_question_gives_none() -> None:
  ...
  sand = Sand(stands=STANDS)
  _, root = life(sand)
  assert engine.peek("bash://nobody", on=root) is None
