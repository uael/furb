...
async def test_a_control_said_over_the_act_it_names_with_a_header_of_its_name() -> None:
  ...
  engine.wake(act)
  await settle()
  assert [(one[1], one[3]) for one in said(log, "pause")] == [(act, ["#wait1 paused"])]
  assert [(one[1], one[3]) for one in said(log, "wake")] == [(act, ["#wait1 woke"])]
...
async def test_the_call_gives_the_control_as_the_life_made_it_whole() -> None:
  ...
  sand = Sand(stands=STANDS)
  log, root = life(sand)
  sand.script[root] = ["b = wait(100)\nk = control('pause', 'paused', b)\nclose(3)"]
  act = engine.prompt(int, "count", on=root)
  assert await act == 3
  await settle()
  step = said(log, "rung")[0][1]
  assert engine.modules[root]["k"] == ("pause", "wait1", step, ["#wait1 paused"])
