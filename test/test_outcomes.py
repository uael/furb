...
async def test_outcomes_holds_what_every_question_of_the_life_came_to_under_its_name() -> None:
  ...
  engine.send("done", act, None, by=WORLD)
  await settle()
  assert act in engine.outcomes and engine.outcomes[act] is None
  got = engine.clock(on=root)
  word = next(one for one in sand.calls if one[0] == "clock")
  assert engine.outcomes[word[1]] == got == 1001.0
...
async def test_the_outcome_is_no_slot_of_the_question() -> None:
  ...
  act = engine.wait(100, on=root)
  await settle()
  assert act not in engine.outcomes and len(said(log, "wait")[0]) == 5
  kept = list(sand.record)
  assert [len(e[0]) for e in kept if e[0][0] == "wait"] == [5]
