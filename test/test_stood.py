...
from conftest import STANDS, Sand, life, plain, relived, said, settle, tags
...
async def test_a_stood_says_what_a_chain_stands_on_from_its_place_in_the_record_on() -> None:
  ...
  await settle()
  assert [e[1][:2] for e in second.record][:1] == [("stood", root)]
  ...
  asked = [a[6] for a in said(heard, "rung")]
  assert asked == ["m/low", "o/low"] and said(heard, "ask") == []
  assert heard.index(stood[0]) < heard.index(said(heard, "rung")[1])
...
async def test_a_chain_that_hears_a_stood_binds_that_standing() -> None:
  ...
  assert tags(now, "opened")[-1][1] == [("id", root), ("roster", LATER[0]), ("directory", "/z"), ("actor", "o/low")]
