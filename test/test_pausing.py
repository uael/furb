...
async def test_an_act_is_paused_while_the_last_control_in_record_order_that_is_over_it_is_a_pause() -> None:
  ...
  engine.wake(root)
  engine.pause(root)
  engine.send("finished", act, 7, by=WORLD)
  await settle()
...
async def test_a_pause_holds_delivery_a_result_that_arrives_enters_the_record_and_waits() -> None:
  ...
  await settle()
  kept = [fact for fact, *_ in sand.record if fact[0] == "finished"]
  assert [one[1] for one in kept] == [act] and act not in engine.outcomes
  engine.wake(root)
  await settle()
  assert (await act) == 0
...
async def test_a_rung_carries_on_only_while_its_own_chain_is_not_paused() -> None:
  ...
  engine.pause(root)
  engine.send("done", waited[1], None, by=WORLD)
  await settle()
  assert said(log, "sent") == []
  engine.wake(root)
  await settle()
  assert [one[1] for one in said(log, "sent")] == [waited[2]] and (await act) == 7
...
async def test_a_control_from_outside_reaches_a_paused_act_at_once() -> None:
  ...
  await settle()
  before = len(heard)
  other = engine.wait(100, on=root)
  engine.close(0, other)
  await settle()
  assert heard[before:] == []
  engine.wake(probe)
  await settle()
  assert [a[:2] for a in heard[before:] if a[0] in ("wait", "close")] == [("wait", other), ("close", other)]
