...
async def test_a_fact_is_a_tuple_its_kind_the_act_it_is_about_who_said_it_and_its_words() -> None:
  ...
  log, root = life(sand)
  act = engine.wait(5.0, on=root)
  word = said(log, "wait")[0]
  match word:
    case ("wait", about, by, on, seconds):
      got = (about, by, on, seconds)
    case _:
      got = None
  assert got == (act, OPERATOR, root, 5.0)
...
async def test_a_fact_says_who_said_it() -> None:
  ...
  assert said(log, "clock")[0][2] == "rung1"
...
async def test_a_fact_is_on_the_scope_of_the_act_it_is_about() -> None:
  ...
  await settle()
  act = engine.wait(0, on=two)
  await act
  theirs = [one for one in log if one[1] == act]
  assert [one[0] for one in theirs if one[0] != "wait" and two in one] == []
  assert (said(log, "wait")[0][3], engine.scope(act)) == (two, two)
...
async def test_a_control_is_about_the_act_it_is_over() -> None:
  ...
  engine.send("tell", act, [f"#{act} noted"])
  engine.cancel(act)
  over = engine.wait(0, on=root)
  await settle()
  assert [one[1] for one in said(log, "cancel")] == [act]
  assert {one[0] for one in log if one[1] == act} == {"wait", "start", "tell", "done", "cancel"}
  told = [one for one in said(log, "tell") if one[1] == act]
  assert [one[3][0] for one in told] == ["#wait1 noted"]
  assert [(one[0], one[2]) for one in log if one[1] == over] == [("wait", OPERATOR), ("start", over), ("done", WORLD)]
...
async def test_a_verb_takes_a_chain_and_the_act_it_makes_is_on_that_chain() -> None:
  ...
  two = engine.chain("two")
  await settle()
  engine.wait(0, on=two)
  engine.wait(0, on=root)
  assert [one[3] for one in said(log, "wait")] == [two, root]
...
async def test_a_model_calls_a_verb_from_a_rung_and_the_operator_calls_the_same_verb_outside_a_rung() -> None:
  ...
  sand = Sand(stands=STANDS)
  log, root = life(sand)
  sand.script[root] = ["x = wait(1.0)\nclose(1)"]
  assert await engine.prompt(int, "wait", on=root) == 1
  engine.wait(2.0, on=root)
  step = said(log, "rung")[0][1]
  assert [(one[4], one[2]) for one in said(log, "wait")] == [(1.0, step), (2.0, OPERATOR)]
...
async def test_an_act_is_on_the_chain_that_the_verb_names() -> None:
  ...
  two = engine.chain("two")
  await settle()
  sand.script[root] = [f"x = wait(0, on={two!r})\nclose(1)"]
  assert await engine.prompt(int, "wait there", on=root) == 1
  assert [one[3] for one in said(log, "wait")] == [two]
...
async def test_the_world_is_given_the_id_of_the_act_and_the_facts_about_the_act_carry_the_same_id() -> None:
  ...
  assert {one[1] for one in log if one[2] == WORLD and one[1] not in engine.asked} == {act}
