...
async def test_a_change_of_the_world_between_two_lives_enters_the_transcript_of_a_chain() -> None:
  ...
  opened = tags(engine.turns(on=over), "opened")
  now = ("opened", [("id", over), ("roster", LATER[0]), ("directory", "/z"), ("actor", "o/low")], None)
  assert opened[1] == was and opened[-1] == now
...
async def test_a_chain_asks_what_it_stands_on_at_its_open_and_the_journal_keeps_that_stand() -> None:
  ...
  assert [e[1:] for e in sand.record if e[1][0] == "stand"] == [(said(log, "stand")[0], STANDS)]
  later = Sand(stands=LATER)
  ...
  assert [a[6] for a in said(again, "rung")] == ["m/low"] and said(again, "ask") == []
...
async def test_at_its_tip_the_journal_asks_the_world_what_it_stands_on() -> None:
  ...
  assert [e[1][:2] for e in later.record] == [("stood", root), ("stood", two), ("stood", side)]
