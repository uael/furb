...
from conftest import STANDS, Sand, gated, job, life, paragraphs, plain, ran, relived, said, settle
...
async def test_a_wake_it_ends_the_pause_over_the_same_act_and_what_waited_is_heard() -> None:
  ...
  await settle()
  engine.pause(act)
  engine.send("finished", act, 3, by=WORLD)
  await settle()
  assert act not in engine.outcomes
  engine.wake(act)
  await settle()
  assert (await act) == 3
  assert paragraphs(engine.turns(on=root))[-2:] == [f"#{act} paused", f"#{act} woke"]
...
async def test_delivery_carries_on_the_rungs_that_await_the_result_on_whatever_chain() -> None:
  ...
  await settle()
  engine.pause(two)
  engine.send("done", which, None, by=WORLD)
  await settle()
  assert act not in engine.outcomes
  engine.wake(two)
  await settle()
...
async def test_a_wake_on_one_act_lifts_a_pause_of_its_chain_for_that_act_alone() -> None:
  ...
  sand = Sand(stands=STANDS)
  _, root = life(sand)
  one, two = job(root), job(root)
  await settle()
  engine.pause(root)
  engine.send("finished", one, 0, by=WORLD)
  engine.send("finished", two, 0, by=WORLD)
  await settle()
  engine.wake(one)
  await settle()
...
async def test_a_wake_that_this_life_says_starts_the_pending_acts_it_is_over() -> None:
  ...
  engine.wake(over)
  await settle()
  assert [a[1] for a in said(later.calls, "start")] == [waited, shown]
