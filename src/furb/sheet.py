...
The word stands on a sheet of its own::
  ...
  import __engine__
  async def __body():
    <each name ty gives a module that the module of a chain does not hold>: object
...
word may await at its top level. ty gives every module the names of IMPLICIT, and the module of a chain holds only
__name__ and those that the module of the engine holds, so the body declares each other one and never binds it:
ty refuses a word that reads it, as the run does. The module of a chain holds every name of the engine, an import
of the engine among them, so the body binds each of them next, with the type the engine gives it. Each is a plain
binding, as it is in the module of a chain, so a word may read a name of the engine and bind it again to any
value, as the contract lets it. The two names a chain binds of its own come next, with the types the contract
gives them. Each word of the program stands in a try of its own, since a rung that raised keeps what it bound
before the raise and the word after it runs all the same: ty reads the next word through the except, with every
name the words before it bound before they raised, so a word that raises at its top level hides no name a later
word binds, and a rung that never ends leaves the word reachable the same way. A call opens each try, since ty
reads an except that no statement before it can reach, and an empty word still has a body. And the program and
the word keep their own lines, so a finding is counted back to the line the model wrote.
...
IMPLICIT = (
  "__name__", "__file__", "__loader__", "__package__", "__path__", "__spec__", "__doc__", "__annotations__",
  "__annotate__", "__builtins__", "__warningregistry__",
)  # fmt: skip
"""IMPLICIT are the names ty gives every module as globals, in module_type_implicit_global_symbol of
ty_python_semantic: the names the typeshed declares in the class types.ModuleType, but __dict__, __init__ and
__getattr__, and __builtins__ and __warningregistry__, which ty adds itself. ty adds __debug__ too, which python
lets no code declare, so it is not here: every chain holds it, python among its builtins and the sandbox in the
...
def chained(engine: dict[str, object], name: str) -> bool:
  """Whether the module of a chain holds the name as python looks up a global: a chain binds __name__ of its own,
  and it holds every name of the module of the engine and of the builtins that module holds, which the sandbox gives
  no module."""
  builtins = engine.get("__builtins__")
  return name == "__name__" or name in engine or (isinstance(builtins, dict) and name in builtins)
...
def sheet(engine: dict[str, object], program: Sequence[str], word: str) -> tuple[str, int]:
  ...
  by: each name of IMPLICIT that the module of a chain does not hold, declared and never bound; every name of the
  engine, which is a key of its module that is not private, bound from MODULE; then the two names the chain binds;
  then each word of the program of the chain before the word, in a try of its own; then the word."""
  unbound = "".join(f"  {name}: object\n" for name in IMPLICIT if not chained(engine, name))
