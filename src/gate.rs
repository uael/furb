...
use ruff_db::{
  Db as SourceDb,
  diagnostic::{
    Diagnostic, DiagnosticFormat, DisplayDiagnosticConfig, DisplayDiagnostics, Severity,
  },
...
use ty_python_semantic::{
  AnalysisSettings, Db, PythonVersionSource, PythonVersionWithSource, check_file_unwrap,
...
impl Reading {
  ...
  fn checked(&mut self, sheet: &str) -> Vec<(usize, String)> {
    ...
    let config =
      DisplayDiagnosticConfig::new("furb").format(DiagnosticFormat::Concise).color(false);
